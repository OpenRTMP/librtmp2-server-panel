...
- **Tests:** unit tests run standalone with `pytest -m "not integration"`. Integration tests are gated by `RUN_INTEGRATION=1` and require a running server (`LRTMP2_API_URL` + `LRTMP2_API_TOKEN`); run them with `pytest -m integration`.
